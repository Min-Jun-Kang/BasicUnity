...
public class Hit_Lazer : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //화면을 클릭했을 때 클릭한 곳을 게임 세상 속으로 적용, 화면을 클릭한 곳이 게임 세계속 원하는 곳이 아님
        MousePos = Input.mousePosition;
        MousePos = Camera.main.ScreenToWorldPoint(MousePos);
        Vector3 Pos = new Vector3(MousePos.x, MousePos.y, 0); //2D 게임이기 때문에 가져온 Z값을 초기화 시킨다.
        dir = Pos - tr.position; //마우스 - 플레이어 포지션 -> 마우스를 바라보는 벡터(A-B)
